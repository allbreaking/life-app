...
const transactionSchema = z.object({ amount: z.number().int(), note: z.string().min(1).max(240) }).strict();
...
export function Finance() {
  const [budgetCents, setBudgetCents] = useDomainResource('finance.budgetCents', z.number().int().nonnegative(), 300_000);
  const [spentCents, setSpentCents] = useDomainResource('finance.spentCents', z.number().int().nonnegative(), 246_000);
  const [pending, setPending] = useDomainResource('finance.pending', pendingSchema, [{ id: 'p1', note: '蓝牙耳机（想换新的，非必需）', amountCents: 12_800 }] as Pending[]);
  const [necessary, setNecessary] = useState(true);
  const [lastTransaction, setLastTransaction] = useDomainResource('finance.lastTransaction', transactionSchema, { amount: -3500, note: '买猫粮 · 自动归类 #宠物耗材' });
