...
- 自定义应用菜单必须同时保留平台标准“编辑”菜单角色：撤销、重做、剪切、复制、粘贴和全选。它们由 Tauri/系统直接路由到当前获得焦点的文本控件，不经过自定义菜单 ID、前端事件或剪贴板 IPC。
...
- 标准编辑动作的副作用仅限当前焦点文本控件及系统剪贴板；不写 SQLite 或文件，不发网络请求，不执行 shell。SOP 等业务数据仍须由用户显式保存后才持久化。
...
4. macOS 文本输入控件可使用系统 `⌘X`、`⌘C`、`⌘V`、`⌘A` 及撤销/重做快捷键；自定义菜单动作白名单保持不变。

## P6-B1a macOS 菜单栏待办状态项

- 应用窗口不再绘制伪 macOS 系统栏：移除左侧星期/时间、顶部快捷录入按钮和顶部待办按钮。
- 页面标题区不展示快捷录入按钮；快捷录入继续由系统级 `⌥ Space` 与原生应用菜单提供。
- “数据保护”保留为页面标题区操作，避免移除伪系统栏时丢失已有能力。
- macOS 桌面壳启动时创建标题为 `□ 14:00 项目周例会` 的系统菜单栏状态项；其他平台不创建该状态项。
- 菜单栏使用短标题 `□ 14:00 周例会`；点击后菜单完整展示 `14:00 项目周例会`，并提供“标记完成”。
- “标记完成”菜单 ID 只在 Rust 内映射到当前固定待办的白名单动作；Rust 即时推进菜单栏短标题和菜单状态，并向 React 发出动作；React 将对应索引幂等写入 `dashboard.completedTodoIndexes`，使应用内待办同步完成。
- 完成当前菜单栏待办后自动展示下一项：`14:00 项目周例会` → `19:00 芭蕾课` → `今日待办已完成`。桌面事件分别使用固定白名单动作 `complete-todo-1`、`complete-todo-2`，不得接受任意索引输入。
- 该操作的显式副作用是更新 macOS 菜单栏瞬时状态、显示/聚焦主窗口、发送桌面事件，以及通过既有 typed IPC/SQLite 领域资源写入完成索引；不发网络请求、不执行 shell、不读写任意文件。

### P6-B1a 验收

1. React 壳层不再出现星期、顶部时间、快捷录入按钮或顶部 `项目周例会` 副本；总览中的真实待办内容不受影响。
2. macOS 状态项使用短标题，点击菜单显示完整待办；完成菜单 ID 经过白名单映射，未知 ID 不产生动作。
3. 点击“标记完成”后菜单栏与菜单立即显示完成状态，React/SQLite 中的对应待办也幂等完成。
4. 完成一项后自动推进下一项；最后一项完成后禁用完成操作并显示当日完成状态。
5. 应用主体占满窗口高度，数据保护入口仍可访问，`⌥ Space` 和原生菜单快捷录入路径保持可用。
