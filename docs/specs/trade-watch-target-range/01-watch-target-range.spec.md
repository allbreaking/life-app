...
---
## 函数：isValidTargetPrices(optimisticTarget, target, pessimisticTarget)
**所在文件**：`src/features/trade/tradeModel.ts`
**输入**：乐观、中枢、悲观三档目标价。
**输出**：仅当三者均为有限正数且 `optimisticTarget >= target >= pessimisticTarget` 时返回 `true`。
**副作用（完整列表）**：
  - 无。
**不应该做的事**：
  - 不得读取、推断或校验安全价。
  - 不得自动排序、修正价格或产生 UI/存储/网络副作用。
**异常处理**：`NaN`、无穷、零、负数或顺序错误返回 `false`。
**幂等性**：是 - 纯函数。
---
...
**副作用（完整列表，没有写“无”）**：
  ...
  - 行情成功后向 `trade.watchlist` 追加包含三档目标价且 `safety: 0` 的完整记录，并通过 typed IPC 幂等替换写入 SQLite。
...
**不应该做的事**：
  ...
  - 不得把乐观价或悲观价作为新增预警阈值。
**异常处理**：
  - 非六位代码、空名称、非有限三档目标价或非法目标价顺序显示页面内中文错误并停止；新增流程不读取或校验安全价。
  - 重复代码显示既有重复提示并停止。
...
**输出**：`JSX.Element` - 依次展示标的、三档目标价、现价、四维评分和状态；安全价仅在编辑态提供。
**副作用（完整列表，没有写“无”）**：
  - 无。
**不应该做的事**：
  - 不得修改观察标的或写入持久化资源。
...
- 组件测试：新增区没有安全价输入；非法三档顺序不调用行情 IPC且不新增记录；合法新增以 `safety: 0` 写入完整字段。
